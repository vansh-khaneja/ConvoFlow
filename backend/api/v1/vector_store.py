...
# Lazy initialization of Qdrant service (only when needed)
...
def get_qdrant_service() -> QdrantService:
    ...
    if _qdrant_service is None:
        try:
            _qdrant_service = QdrantService(**VectorStoreConfig.get_qdrant_config())
            logger.info("Qdrant service initialized successfully")
        except Exception as e:
            logger.warning(f"Failed to initialize Qdrant service: {e}")
            logger.warning("Qdrant service will be unavailable. Endpoints will return errors.")
            # Re-raise to let endpoints handle it appropriately
            raise RuntimeError(f"Qdrant service is not available: {str(e)}. Please check your Qdrant connection.")
