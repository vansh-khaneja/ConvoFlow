...
def init_db() -> None:
    """Initialize deployments tables. Called on app startup."""
    try:
        ...
        print(f"Warning: Failed to initialize deployments database: {e}")
...
NODE_ID_TO_CLASS: Dict[str, str] = {
    ...
    "document_loader_node": "DocumentLoaderNode",
}
...
async def delete_deployment(deployment_id: str):
    with get_connection() as conn:
        # First delete all logs associated with this deployment
        conn.execute(
            "DELETE FROM deployment_logs WHERE deployment_id = %s"
            if USING_POSTGRES
            else "DELETE FROM deployment_logs WHERE deployment_id = ?",
            (deployment_id,),
        )
