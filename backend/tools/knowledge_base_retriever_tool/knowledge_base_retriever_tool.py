...
class KnowledgeBaseRetrieverTool:
    ...
    def search_documents(
        ...
        """
        try:
            # Check for configuration issues and provide specific error messages
            issues = []
            
            # Check Qdrant service
            if not self.qdrant_service:
                issues.append("Qdrant service not initialized")
            elif not hasattr(self.qdrant_service, 'client') or self.qdrant_service.client is None:
                issues.append("Qdrant not connected - check QDRANT_URL and ensure Qdrant server is running")
            
            # Check OpenAI client
            if not self.openai_client:
                issues.append("OpenAI API key not set - check OPENAI_API_KEY environment variable")
            
            # Return combined error message if there are issues
            if issues:
                if len(issues) == 1:
                    error_msg = issues[0]
                else:
                    error_msg = "Multiple configuration issues: " + "; ".join(issues)
