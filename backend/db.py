...
POSTGRES_URL = os.environ.get("POSTGRES_DB_URL_BASE")
...
# Only create data directory and SQLite path if not using Postgres
# Store database in backend directory for standalone deployment
DATA_DIR = Path(__file__).resolve().parent / "data"
SQLITE_DB_PATH = DATA_DIR / "workflows.db"

# Only create data directory if using SQLite (lazy creation)
...
def _get_postgres_pool() -> "ConnectionPool":
    ...
    if _postgres_pool is None:
        ...
        if not POSTGRES_URL:
            raise RuntimeError(
                "POSTGRES_DB_URL_BASE environment variable is not set. "
                "Please set it in your .env file or unset it to use SQLite instead."
            )
        ...
        try:
            _postgres_pool = ConnectionPool(
                POSTGRES_URL,
                min_size=1,  # Reduced from 2 to avoid blocking on startup
                max_size=10,
                timeout=5.0,
                kwargs={
            ...
            raise RuntimeError(
                f"Failed to create Postgres connection pool. "
                f"Please check that POSTGRES_DB_URL_BASE is correct and the database is running. "
                f"Error: {str(e)}"
            ) from e
...
def get_connection() -> Iterator["ConnectionProtocol"]:
    ...
    if USING_POSTGRES:
        ...
        # Only create data directory when actually using SQLite
        _ensure_data_dir()
